# Cite the paper from which the formula come
...
class MultiHeadGeneralizedPooling(nn.Module):
    def __init__(self, token_dim: int = 768, sentence_dim: int = 512, num_heads: int = 8, mean_pooling_init=True):
        """
        ...
        self.hidden_dim = int(sentence_dim / self.num_heads)
        self.sentence_dim = sentence_dim
        self.token_dim = token_dim

        # Define learnable weights and biases for each head
        self.W1 = nn.ModuleList([nn.Linear(token_dim, self.hidden_dim) for _ in range(num_heads)])  # W1^i for each head
        self.W2 = nn.ModuleList([nn.Linear(self.hidden_dim, token_dim) for _ in range(num_heads)])  # W2^i for each head
    ...
    def forward(self, token_embeddings: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.num_heads):
            ...
            H = token_embeddings # (batch_size, seq_len, token_dim)
            H = self.W1[i](H)  # (batch_size, seq_len, hidden_dim) for head i
            H = F.relu(H)  # Apply ReLU activation

            # Second linear transformation: W2^i * ReLU(W1^i * H^T + b1^i)
            A_i = self.W2[i](H)  # (batch_size, seq_len, token_dim) for head i
            A_i = A_i.transpose(2, 1) # (batch_size, token_dim, seq_len) for head i
            ...
            A_i = A_i.unsqueeze(-1)
            v_i = torch.sum(token_embeddings * A_i, dim=1)  # Weighted sum over seq_len (batch_size, token_dim)
        ...
        assert pooled_output.shape[1] == self.token_dim
